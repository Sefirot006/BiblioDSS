...
public System.Collections.Generic.IList<ReservaEN> ListaReservas(int first, int size)
{
    ...
    try
    {
        ...
        if (size > 0)
        {
            ...
            {
                for (int j = 0; j < result[i].Obra.Count; j++)
                {
                    Console.WriteLine(result[i].Obra[j].Tematica.Count);
                    Console.WriteLine(result[i].Obra[j].Escrita.Count);
                }

                Console.WriteLine(result[i].Usuario);
                Console.WriteLine(result[i].IdReserva);
            }
        }
        else
        {
